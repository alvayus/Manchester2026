...
Artix UltraScale Plus devices (XEM8320)
-----------------------------------------------------
The AMD Memory Interface Generator (MIG) is used to create the DDR4
controller for this design. We have provided a project.tcl script to
configure the MIG as well as the FIFO IPs used within this project.

Open the project.tcl file and follow the instructions within the header
to build the project within the Vivado version of your choosing. You
need only to import your product's FrontPanel HDL to generate the bitstream.

You can also see the users manual for a walkthrough of setting up the memory
controller. Note that different versions of the IP may generate products
that are not fully compatible with this sample. Some updates to the sample
may be required to bring it up to date with the latest output from this tool.

Kintex UltraScale devices (XEM8350)
-----------------------------------------------------
The AMD Memory Interface Generator (MIG) is used to create the DDR4
controller for this design. We have provided a project.tcl script to
configure the MIG as well as the FIFO IPs used within this project.
...
Artix 7 devices (XEM7010, XEM7310, XEM7320)
-----------------------------------------------------
The AMD Memory Interface Generator (MIG) is used to create the DDR3
...
that are not fully compatible with this sample. Some updates to the sample
may be required to bring it up to date with the latest output from this tool.
AMD recommends using Vivado for designs using memory interfaces on
7-series devices.

Kintex 7 devices (XEM7350, XEM7360)
-----------------------------------------------------
The AMD Memory Interface Generator (MIG) is used to create the DDR3
controller for this design. To build this sample from sources you will
need to generate this memory controller using the MIG IP in Vivado.
...
------------------------------------
The Intel Hard Memory Controller is used to create the DDR3 controller
...
----------------------------
The Intel High Performance Controller (HPC) is used to create the DDR2
controller for this design.

Spartan-6 devices (XEM6110, XEM6010)
------------------------------------
The AMD Memory Interface Generator (MIG) is used to create the DDR2 
controller for this design.
